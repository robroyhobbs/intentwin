...
export function IntelStats({ intelligence }: IntelStatsProps) {
  ...
  if (intelligence.agency_avg_offers !== null) {
    stats.push({ value: String(intelligence.agency_avg_offers), label: "Avg Offers" });
  }
  if (intelligence.agency_avg_amount !== null) {
    stats.push({ value: formatCurrency(intelligence.agency_avg_amount), label: "Avg Award" });
  }
  ...
  if (intelligence.agency_total_awards !== null && stats.length < 4) {
    stats.push({ value: String(intelligence.agency_total_awards), label: "Awards Tracked" });
  }

  return (
    <div data-testid="intel-stats" className="rounded-lg border border-border bg-card p-3">
